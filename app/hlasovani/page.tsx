      ...
      <main className="pt-24 pb-32 px-4 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            { icon: "people",      label: "Celkem hlasů",   value: polls.reduce((s, p) => s + p.totalVotes, 0).toLocaleString("cs-CZ") },
        ...
        <div className="space-y-6">
          {polls.map((poll) => {
            ...
            const options    = poll.options.map((o) => ({
              ...o,
              votes: hasVoted && o.id === voted[poll.id] ? o.votes + 1 : o.votes,
            }));
            ...
            <div key={poll.id} className="rounded-[2rem] overflow-hidden"
                ...
                <div className="px-6 pt-6 pb-4">
                  ...
                  <h2 className="font-headline font-bold text-lg leading-snug mb-1"
                ...
                <div className="px-6 pb-6 space-y-3">
                  {options.map((opt) => {
                    ...
                    return (
                      <div key={opt.id}>
                        {!hasVoted ? (
                          /* Before voting — selectable option */
                          <button
                        ...
                        style={
                          ...
                          >
                            <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all"
                              ...
                              {isSelected && (
                                <div className="w-2 h-2 rounded-full" style={{ background: "var(--on-primary)" }} />
                              )}
                            </div>
                            <span className="font-medium text-sm">{opt.text}</span>
                          </button>
                        ) : (
                          /* After voting — progress bar */
                          <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium flex items-center gap-2"
                                    style={{ color: isLeading ? "var(--primary)" : "var(--on-surface)" }}>
                                {isVotedFor && (
                              ...
                              <span className="font-headline font-black text-sm"
                                    style={{ color: isLeading ? "var(--primary)" : "var(--on-surface-variant)" }}>
                                {pct}%
                              </span>
                            </div>
                            <div className="h-2.5 rounded-full overflow-hidden"
                              ...
                              <div
                                ...
                                style={{
                                  width: `${pct}%`,
                                  background: isLeading ? "var(--primary)" : "var(--secondary)",
                                }}
                              />
                            </div>
                          </div>
        ...
        <div className="mt-8 rounded-[2rem] p-6 relative overflow-hidden"
          ...
          <div className="relative">
            ...
            <h3 className="font-headline font-extrabold text-xl mb-1" style={{ color: "var(--on-secondary)" }}>
              Participativní rozpočet
            </h3>
            <p className="text-sm mb-5" style={{ color: "var(--secondary-fixed)", opacity: 0.85 }}>
              Vimperk plánuje otevřít hlasování o&nbsp;využití 500 000 Kč z&nbsp;městského rozpočtu. Sledujte novinky.
            </p>
            <button className="px-5 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all active:scale-95"
                    style={{ background: "var(--on-secondary)", color: "var(--secondary)" }}>
